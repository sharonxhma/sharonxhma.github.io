...
export function LinkButton({
  ...
  switch (icon) {
    case "link":
      iconComponent = (
        <LinkIcon
          className="hidden md:inline-block -ml-0.5 h-5 w-5"
          aria-hidden="true"
        />
      );
      break;
  ...
  return (
    <Link href={url} target="_blank">
      <button
        type="button"
        className="px-3 xl:px-4 py-2 xl:py-2.5 inline-flex items-center gap-x-1.5 rounded border border-primary-700 text-sm font-medium text-primary-700 shadow-sm hover:bg-primary-300"
      >
        {iconComponent}
        {text}
      </button>
